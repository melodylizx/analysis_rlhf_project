...
def get_percent_dataset(pct):
    ...
    num_train = int(pct * len(train_comparisons))
    num_valid = int(pct * len(valid_comparisons))

    # Sample the dataset
    return_train = train_comparisons.sample(n=num_train)
    return_valid = valid_comparisons.sample(n=num_train)
