...
class PairwiseDataset(Dataset):
    def __init__(self, pairs, tokenizer, max_length):
        ...
        for pair in pairs:
            ...
            if True or (not torch.all(torch.eq(chosen_encodings_dict["input_ids"], rejected_encodings_dict["input_ids"])).item()):
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(directory_path, "train_outputs.pkl")):
        ...
        train_dataloader = DataLoader(train_dataset, shuffle=False, batch_size=14, collate_fn=DataCollatorReward())
        ...
        for i in range(5):
            ...
            raise RuntimeError("Failed to load train_outputs.pkl after multiple attempts.")
    ...
    if not os.path.exists(os.path.join(directory_path, "val_outputs.pkl")):
        ...
        val_dataloader = DataLoader(val_dataset, shuffle=False, batch_size=23, collate_fn=DataCollatorReward())
        ...
        print(" >>>>>>>>>> len val_ids", len(val_outputs))
        print("########### saving outputs to .pkl")
        ...
        for i in range(5):
            ...
            raise RuntimeError("Failed to load val_outputs.pkl after multiple attempts.")
            
    print("########### computing uncertainty scores")
                
    train_ids_with_uncertainty_score = []
    val_ids_with_uncertainty_score = []
    for idx, chosen_score, rejected_score in zip(train_outputs["id"], train_outputs["chosen_end_score"], train_outputs["rejected_end_score"]):
    ...
    for idx, chosen_score, rejected_score in zip(train_outputs["id"], val_outputs["chosen_end_score"], val_outputs["rejected_end_score"]):
        uncertainty_score = measure_uncertainty_method(torch.tensor([chosen_score, rejected_score]))
        val_ids_with_uncertainty_score.append((idx, uncertainty_score))
    print(print(" >>>>>>>>>> len val_ids", len(val_ids_with_uncertainty_score)))
    print("########### selecting uncertain samples")
    # sort ids by decreasing uncertainty
    print(" >>>>>>>>>> len val_ids unique",len(pd.DataFrame(val_ids_with_uncertainty_score,columns=['ids','uncertainty']).ids.unique()))
    ...
    print(" >>>>>>>>>> len(val_ids_with_uncertainty_score)", len(val_ids_with_uncertainty_score))
    print(" >>>>>>>>>> len(val_output_dataset[prompt])", len(val_output_dataset["prompt"]))
    print(" >>>>>>>>>> len(train_ids_with_uncertainty_score)", len(train_ids_with_uncertainty_score))
    print(" >>>>>>>>>> len(train_output_dataset[prompt])", len(train_output_dataset["prompt"]))
    ...
    train_output_dataset.to_pandas().to_parquet(os.path.join(directory_path, "train_uncertain.parquet"), index=False)
    val_output_dataset.to_pandas().to_parquet(os.path.join(directory_path, "validation_uncertain.parquet"), index=False)
    print("########### done")
